...
The databricks CLI bundles top level is defined by file `ngm_mlops/databricks.yml`.
...
2. Complete the `TODO` in `ngm_mlops/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `ngm_mlops/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `ngm_mlops/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-ngm_mlops-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-ngm_mlops-batch-inference-job`
that runs the notebook under `ngm_mlops/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-ngm_mlops-batch-inference-job`.
...
`test-ngm_mlops-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-ngm_mlops-batch-inference-job`.
